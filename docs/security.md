...
- All production client traffic uses HTTPS. Elastic Beanstalk single-instance serves HTTP on port 80; TLS is terminated at **API Gateway HTTP API** in front of EB (workaround because CloudFront distributions cannot be created until this AWS account is verified). See Architecture Decision #24.
...
- **Rate limiting:** Request throttling is enforced in the NestJS application via `@nestjs/throttler` (global per-IP limit plus a stricter limit on `POST /auth/login`). API Gateway is used only as an HTTPS proxy; usage plans / WAF throttling are not used. Configure via `THROTTLE_TTL_MS`, `THROTTLE_LIMIT`, and `LOGIN_THROTTLE_LIMIT` in environment variables.
